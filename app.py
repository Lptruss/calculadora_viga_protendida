...
def executar_analise(L=600, bw=20, h=60, f=8, z_offset=31, div_x=30, div_y=2, 
    ...
    _, tensoes_trel_final, _ = calcular_esforcos_trelica(IE_trelica, XYZ, U_total_final, E_trelica, A_trelica)

    df_final_trelica = resultados_protensao["detalhamento_perdas"].copy()
    df_final_trelica['Tensao_Inicial_kN_cm2'] = sigma_protensao
    df_final_trelica['Tensao_Axial_Elastica_kN_cm2'] = tensoes_trel_final
    
    tensao_efetiva = (
        sigma_protensao 
        - df_final_trelica['Perda_Tensao_Atrito_kN_cm2'] 
        - df_final_trelica['Perda_Tensao_Ancoragem_kN_cm2'] 
        - df_final_trelica['Perda_Tensao_CS_Flu_kN_cm2'] 
        - df_final_trelica['Perda_Tensao_Relaxacao_kN_cm2'] 
        + tensoes_trel_final
    )
    df_final_trelica['Tensao_Final_Efetiva_kN_cm2'] = tensao_efetiva
    
    excel_filename = 'resultados_viga.xlsx'
    with pd.ExcelWriter(excel_filename) as writer:
        XYZ_final = U_total_final.reshape((-1, 3))
        dados_nos = np.hstack([XYZ, XYZ_final])
        df_nos = pd.DataFrame(dados_nos, columns=['X', 'Y', 'Z', 'Ux', 'Uy', 'Uz'])
        df_nos.to_excel(writer, sheet_name='Nos_e_Deslocamentos')
        df_final_trelica.to_excel(writer, sheet_name='Resultados_Completos_Trelica')

    return flecha_no_centroide, excel_filename# -*- coding: utf-8 -*-
"""
Calculadora de Viga Protendida - MEF 3D (Sólidos + Treliça)
Versão Segura e Descompactada
"""
...
def executar_analise(L=600, bw=20, h=60, f=8, z_offset=31, div_x=30, div_y=2, 
    ...
    return flecha_no_centroide, excel_filename
